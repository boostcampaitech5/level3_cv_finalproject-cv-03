...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if tokenizers is not None:
            self.instance_prompt_tokens_one = tokenize_prompt(
                self.tokenizer_one, self.instance_prompt
            )
            self.instance_prompt_tokens_two = tokenize_prompt(
                self.tokenizer_two, self.instance_prompt
            )
            if class_data_root is not None:
                self.class_prompt_tokens_one = tokenize_prompt(
                    self.tokenizer_one, self.class_prompt
                )
                self.class_prompt_tokens_two = tokenize_prompt(
                    self.tokenizer_two, self.class_prompt
                )
...
def collate_fn(examples, train_text_encoder=False, with_prior_preservation=False):
    ...
    if with_prior_preservation:
        ...
        add_time_ids += [
            example["class_added_cond_kwargs"]["time_ids"] for example in examples
        ]

        if not train_text_encoder:
            input_ids += [example["class_prompt_ids"] for example in examples]
            add_text_embeds += [
                example["class_added_cond_kwargs"]["text_embeds"]
...
def main(args):
    ...
    if not args.train_text_encoder:
        ...
        instance_unet_added_conditions.update(
            {"text_embeds": instance_pooled_prompt_embeds}
        )
    ...
    if args.with_prior_preservation:
        ...
        if not args.train_text_encoder:
            ...
            class_unet_added_conditions.update(
                {"text_embeds": class_pooled_prompt_embeds}
            )
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        collate_fn=lambda examples: collate_fn(
            examples, args.train_text_encoder, args.with_prior_preservation
        ),
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if not args.train_text_encoder:
                    ...
                    batch["unet_added_conditions"].update(
